...
for episode in range(episodes):
    ...
    while not done:
        ...
        #s = np.concatenate([state["scores"], [state["turn"]], state["bobombs"], state["phase"]])
        s_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)

        # Action (with Epsilon Greedy)
        actionList = []
        ...
        # Store transition
        #ns = np.concatenate([next_state["scores"], [next_state["turn"]], next_state["bobombs"], state["phase"]])
